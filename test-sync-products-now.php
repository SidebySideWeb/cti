...
if ( ! $endpoint ) {
	...
	echo '<p>Before syncing products, you need to configure the SoftOne API settings.</p>';
...
<html>
<head>
	<title>Force Full Product Sync</title>
	<style>
...
<div class="container">
	<h1>🔄 Force Full Product Sync</h1>
	
	<div class="info-box">
		<strong>Status:</strong> Sync is running...<br>
		This will synchronize all products from SoftOne ERP to WordPress, ignoring last sync time.
	</div>
...
// Get count before
$count_before = $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->posts} WHERE post_type = 'product'" );

echo '<div class="status loading">';
echo '<strong>Before sync:</strong> ' . $count_before . ' products<br>';
echo '<strong>Starting force full sync...</strong>';
echo '</div>';

// Capture sync output
ob_start();
\S1WC\Product_Sync::sync_products( true ); // Force full sync
$output = ob_get_clean();

// Get count after
$count_after = $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->posts} WHERE post_type = 'product'" );
...
if ( $new_count >= 0 ) {
	...
	echo 'New/updated products: <strong>' . $new_count . '</strong><br>';
	echo 'Total products: <strong>' . $count_after . '</strong>';
	echo '</div>';
} else {
	echo '<div class="status error">';
	echo '✗ <strong>Sync may have encountered issues</strong><br>';
	echo 'Check the debug logs below for details.';
	echo '</div>';
}
...
$list = \S1WC\Settings::get('browser_items', 'CtiWSItems');

echo '<h3>Testing API Connection</h3>';
$res = $api->get_browser_rows( 'ITEM', $list, '', 5, 0 );
...
if ( is_wp_error( $res ) ) {
	...
	echo 'Total products in ERP: <strong>' . ( $res['totalcount'] ?? 0 ) . '</strong><br>';
