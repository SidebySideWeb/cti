;( function( elementor, $, window ) {
	...
	var TemplateLibraryInsertTemplateBehavior = Marionette.Behavior.extend({
		...
		onInsertButtonClick: function onInsertButtonClick() {
			...
			$e.run('glozin_ext/insert-template', args);
		}
	});
	...
	var TemplateLibraryTemplateGlozinExtView = TemplateLibraryTemplateView.extend({
		template: '#tmpl-elementor-template-library-template-glozin-ext',
		ui: function ui() {
		...
		onFavoriteCheckboxChange: function onFavoriteCheckboxChange() {
			...
			glozin_ext_templates_lib.templates.markAsFavorite(this.model, isFavorite);

			if (!isFavorite && glozin_ext_templates_lib.templates.getFilter('favorite')) {
	...
	var TemplateLibraryHeaderPreviewView = Marionette.ItemView.extend({
		template: '#tmpl-elementor-template-library-header-preview-glozin-ext',
	...
	var TemplateLibraryHeaderBackView = Marionette.ItemView.extend( {
		...
		onClick: function() {
			$e.routes.restoreState( 'glozin_ext' );
		},
	} );

	var TemplateLibraryCollectionView = Marionette.CompositeView.extend({
		template: '#tmpl-elementor-template-library-templates-glozin-ext',
		...
		getChildView: function getChildView(childModel) {
			return TemplateLibraryTemplateGlozinExtView;
		},
		initialize: function initialize() {
			this.listenTo(elementor.channels.templates, 'filter:change', this._renderChildren);
		},
		filter: function filter(childModel) {
			var filterTerms = glozin_ext_templates_lib.templates.getFilterTerms(),
				passingFilter = true;
			jQuery.each(filterTerms, function (filterTermName) {
				var filterValue = glozin_ext_templates_lib.templates.getFilter(filterTermName);
		...
		addSourceData: function addSourceData() {
			...
			this.$el.attr('data-template-source', isEmpty ? 'empty' : glozin_ext_templates_lib.templates.getFilter('source'));
		},
		...
		toggleFilterClass: function toggleFilterClass() {
			this.$el.toggleClass('elementor-templates-filter-active', !!(glozin_ext_templates_lib.templates.getFilter('text') || glozin_ext_templates_lib.templates.getFilter('favorite')));
		},
		onRender: function onRender() {
			if ('glozin_ext' === glozin_ext_templates_lib.templates.getFilter('source')) {
				this.setFiltersUI();
			}
		},
		onRenderCollection: function onRenderCollection() {
			...
			if ('glozin_ext' === glozin_ext_templates_lib.templates.getFilter('source') && 'page' !== glozin_ext_templates_lib.templates.getFilter('type')) {
				this.setMasonrySkin();
			}
		},
		...
		onTextFilterInput: function onTextFilterInput() {
			glozin_ext_templates_lib.templates.setFilter('text', this.ui.textFilter.val());
		},
		onSelectFilterChange: function onSelectFilterChange(event) {
			var $select = jQuery(event.currentTarget),
				filterName = $select.data('elementor-filter');
			glozin_ext_templates_lib.templates.setFilter(filterName, $select.val());
		},
		onMyFavoritesFilterChange: function onMyFavoritesFilterChange() {
			glozin_ext_templates_lib.templates.setFilter('favorite', this.ui.myFavoritesFilter[0].checked);
		},
	...
	var TemplateLibraryModalLogoView = Marionette.ItemView.extend({
		template: '#tmpl-elementor-templates-modal__header__logo-glozin-ext',
		className: function className() {
	...
	var TemplateLibraryHeaderActionsView = Marionette.ItemView.extend({
		template: '#tmpl-elementor-template-library-header-actions-glozin-ext',
		...
		onSyncClick: function() {
			...
			glozin_ext_templates_lib.templates.requestLibraryData({
				onUpdate: function onUpdate() {
					self.ui.sync.removeClass('eicon-animation-spin');
					$e.routes.refreshContainer('glozin_ext');
				},
				forceUpdate: true,
				forceSync: true
			});
	...
	var TemplateLibraryHeaderMenuView = Marionette.ItemView.extend({
		...
		templateHelpers: function templateHelpers() {
			return {
				tabs: $e.components.get('glozin_ext').getTabs()
			};
		}
	});
	...
	var TemplateLibraryLayoutView = elementorModules.common.views.modal.Layout.extend({
		...
		getLogoOptions: function() {
			return {
				title: 'Glozin Library',
				click: function click() {
					$e.run('glozin_ext/open', {
						toDefault: true
					});
					return false;
				}
	...
	var Component = function(ComponentModal) {
		...
		Component.prototype.show = function( args ) {
			...
			if ( args.toDefault || ! $e.routes.restoreState( 'glozin_ext' ) ) {
	...
	var TemplateLibraryManager = function TemplateLibraryManager() {
		...
		this.requestLibraryData = function (options) {
			...
			elementorCommon.ajax.addRequest('glozin_ext_get_library_data', ajaxOptions);
		};
		...
		this.filterTemplates = function () {
			return templatesCollection.filter(function (model) {
				if ('glozin_ext' !== model.get('source')) {
					return false;
				}
	...
	glozin_ext_templates_lib.templates = new TemplateLibraryManager();
	glozin_ext_templates_lib.templates.init();
	...
	elementor.on('document:loaded', function() {
		...
		var $templateLibBtnStyle = $( '<style />' ),
			btnStyle = '';
			btnStyle += '.elementor-add-section-area-button.glozin-ext-add-template-button { padding: 0; vertical-align: bottom; background-color: #004bef; }';
			btnStyle += '.elementor-add-section-area-button.glozin-ext-add-template-button img { height: 40px; }';
		...
		$templateLibBtn.addClass( 'elementor-add-section-area-button glozin-ext-add-template-button' );
		$templateLibBtn.attr( 'title', 'Add Glozin Template' );
		$templateLibBtn.html( '<img src="' + glozin_ext_templates_lib.logoUrl + '" />' );
		...
		$previewContents.find('body').delegate('.elementor-editor-element-add', 'click', function(e) {
			setTimeout(function() {
				...
				if ( sectionInner.length > 0 ) {
					sectionInner.find( '.glozin-ext-add-template-button' ).remove();
